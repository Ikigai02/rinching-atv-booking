...
router.get("/availability", (req, res) => {
  ...
  if (date < today) return res.json({ blockedDay: "That date has already passed.", slots: [] });
...
router.post("/", requireRole("customer"), (req, res) => {
  ...
  const today = new Date().toISOString().slice(0, 10);
  if (date < today) return res.status(400).json({ error: "Cannot book a date in the past." });
...
// ---------------------------------------------------------------------------
router.post("/:id/pay", requireRole("customer"), (req, res) => {
  sched.expireStaleHolds();
  ...
  if (!["card", "bank_transfer"].includes(method)) {
    return res.status(400).json({ error: "method must be 'card' or 'bank_transfer'." });
  }
  ...
  if (method === "card") {
    // Simulated payment gateway (no real processor is wired up in this demo).
    const info = db
      .prepare(
